...
const useVolunteerStore = common_vendor.defineStore("volunteer", () => {
  ...
  async function fetchActivityList(page = 1, pageSize = 10) {
    const result = await api_mock.getActivityList({
      page,
      pageSize,
  ...
  async function fetchMySignups() {
    mySignups.value = await api_mock.getMySignups();
  }
  async function submitCheckin(form) {
    const record = await api_mock.submitCheckin(form);
  ...
  async function fetchMyRecords(page = 1, pageSize = 10) {
    const result = await api_mock.getMyRecords({ page, pageSize });
    if (page === 1) {
  ...
  async function fetchStatistics(params) {
    statistics.value = await api_mock.getStatistics();
  }
  async function exportReport(params) {
    const { downloadUrl } = await api_mock.exportReport();
    return downloadUrl;
  }
