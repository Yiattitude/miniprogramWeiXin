  ...
  setup(__props) {
    ...
    common_vendor.onLoad(async () => {
      try {
        ...
        common_vendor.index.showToast({ title: (err == null ? void 0 : err.message) || "鍔犺浇澶辫触", icon: "none" });
      } finally {
        loading.value = false;
      }
    });
    return (_ctx, _cache) => {
      return common_vendor.e({
        ...
        c: common_vendor.t(data.value.totalActivities != null ? data.value.totalActivities : 0),
        d: common_vendor.t(data.value.totalSignups != null ? data.value.totalSignups : 0),
        e: common_vendor.t(data.value.totalCheckins != null ? data.value.totalCheckins : 0),
        f: common_vendor.t(data.value.totalHours != null ? data.value.totalHours : 0),
        ...
        i: common_vendor.f(data.value.activities, (item, k0, i0) => {
          return {
            ...
            b: item.name,
            c: common_vendor.t(item.signupCount != null ? item.signupCount : 0),
            d: common_vendor.t(item.checkinCount != null ? item.checkinCount : 0),
            e: common_vendor.t(item.totalHours != null ? item.totalHours : 0),
            f: item._id
          };
        })
      }));
