...
const _sfc_main = {
  ...
  mixins: [common_vendor.mpMixin, common_vendor.mixin, common_vendor.mixin_accept, common_vendor.props$3],
  data() {
    return {
      lists: [],
      isInCount: true
  ...
  methods: {
    formatFileList() {
      ...
      const lists = fileList.map(
        (item) => Object.assign(Object.assign({}, item), {
          ...
          isImage: this.accept === "image" || common_vendor.image(item.url || item.thumb),
          isVideo: this.accept === "video" || common_vendor.video(item.url || item.thumb),
    ...
    chooseFile() {
      ...
      this.timer = setTimeout(() => {
        ...
        let capture;
        try {
          capture = common_vendor.array(this.capture) ? this.capture : this.capture.split(",");
        } catch (e) {
          capture = [];
        }
    ...
    onBeforeRead(file) {
      ...
      let res = true;
      if (common_vendor.func(beforeRead)) {
    ...
    onPreviewImage(item, index) {
      ...
      common_vendor.index.previewImage({
        ...
        urls: this.lists.filter((item2) => this.accept === "image" || common_vendor.image(item2.url || item2.thumb)).map((item2) => item2.url || item2.thumb),
        current: findIndex,
        fail() {
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    l: common_vendor.sr("previewVideo", "9d90699b-6"),
    m: common_vendor.s(_ctx.$uv.addStyle(_ctx.customStyle))
  });
}
const Component = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-9d90699b"]]);
