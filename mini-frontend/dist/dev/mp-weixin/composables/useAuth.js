...
function useAuth() {
  ...
  function handleWeChatLogin(redirect) {
    return __async(this, null, function* () {
      loading.value = true;
      error.value = null;
      try {
        ...
        if (loginRes.code) {
          const authResult = yield api_user.api.wechatLogin({ code: loginRes.code });
  ...
  function bindUserInfo(realName, phone) {
    return __async(this, null, function* () {
      loading.value = true;
      error.value = null;
      try {
        const bindRes = yield api_user.api.bindUser({
