  ...
  <view class="bind-page">
    ...
    <form class="form-wrap" @submit="onSubmit">
      ...
      <view class="form-actions">
        <button class="submit-btn" form-type="submit" :disabled="loading" :loading="loading">
          {{ loading ? '验证中...' : '确认绑定' }}
        </button>

        <button class="cancel-btn" @tap="onCancel" :disabled="loading">取消</button>
      </view>
    </form>
...
function validateRealName(value: string) {
  if (!value.trim()) errors.realName = '请输入真实姓名'
  else if (value.length < 2) errors.realName = '姓名至少2个字符'
  else if (!/^[\u4e00-\u9fa5]{2,}$/.test(value)) errors.realName = '请输入正确的中文姓名'
  else errors.realName = ''
}

function validatePhone(value: string) {
  if (!value) errors.phone = '请输入手机号码'
  else if (!/^1[3-9]\d{9}$/.test(value)) errors.phone = '请输入正确的手机号码'
  else errors.phone = ''
}
...
async function onSubmit() {
  if (!openid.value) {
    uni.showToast({ title: '缺少 openid，请重新登录', icon: 'none' })
    uni.redirectTo({ url: `/pages/auth/login?redirect=${encodeURIComponent(redirect.value)}` })
    return
  }
  ...
  loading.value = true
  try {
    const res = await bindUser({ openid: openid.value, realName: form.realName, phone: form.phone })
